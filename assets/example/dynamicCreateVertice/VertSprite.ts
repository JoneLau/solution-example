import { _decorator, CCBoolean, CCInteger, cclegacy, InstanceMaterialType, Material, NodeEventType, RenderTexture, Sprite, SpriteAtlas, SpriteFrame, UIRenderer } from "cc";
import { BUILD, EDITOR } from "cc/env";
import { VertSpriteAssembler } from "./VertSpriteAssembler";
...
@ccclass("VertSprite")
export class VertSprite extends UIRenderer {
   ...
   */
  changeSpriteFrameFromAtlas(name: string) {
    if (!this._atlas) {
  ...
  changeMaterialForDefine() {
    ...
    if (texture instanceof cclegacy.TextureBase) {
      ...
      value =
        format === cclegacy.TextureBase.PixelFormat.RGBA_ETC1 ||
        format === cclegacy.TextureBase.PixelFormat.RGB_A_PVRTC_4BPPV1 ||
        format === cclegacy.TextureBase.PixelFormat.RGB_A_PVRTC_2BPPV1;
    }

    if (value && this.grayscale) {
      this._instanceMaterialType =
        InstanceMaterialType.USE_ALPHA_SEPARATED_AND_GRAY;
    } else if (value) {
  ...
  protected _flushAssembler() {
    const assembler = VertSpriteAssembler;
  ...
  //   vertRows = Math.floor(100 / 2.5) + 1;
  // vertRows = 11;
  /**
   * 行数
   */
  @property({ type: CCInteger, serializable: true, displayName: "行数", tooltip: "顶点的行数，默认是11行" })
  vertRows = 11;
  /**
   * 列数
   */
  @property({ type: CCInteger, serializable: true, displayName: "列数", tooltip: "顶点的列数，默认是11列" })
  vertCols = 11;
}
